...
public class VshardedJedis extends ShardedJedis {
    ...
    public Long move(final String key, final int dbIndex) {
        Long result = 0l;
        for(Jedis j:this.getAllShards()){
            result= result | j.move(key, dbIndex);
        }
    ...
    public Long expire(final String key, final int seconds) {
        Long result = 0l;
        for(Jedis j:this.getAllShards()){
            result= result | j.expire(key, seconds);
        }
    ...
    public Long expireAt(final String key, final long unixTime) {
        Long result = 0l;
        for(Jedis j:this.getAllShards()){
            result= result | j.expireAt(key, unixTime);
        }
    ...
    public Long pexpire(final String key, final int milliseconds) {
        Long result = 0l;
        for(Jedis j:this.getAllShards()){
            result= result | j.pexpire(key, milliseconds);
        }
    ...
    public Long pexpireAt(final String key, final long millisecondsTimestamp) {
        Long result = 0l;
        for(Jedis j:this.getAllShards()){
            result= result | j.pexpireAt(key, millisecondsTimestamp);
        }
    ...
    public List<Object> spopEx(VshardedJedisPool pool,String...keys){
        List<Object> result = new ArrayList<Object>();

        for(int i=0;i<getAllShards().size();i++){
            if(result.size() < keys.length){
                int cout = keys.length-result.size();
                String [] tmpKeys = new String[cout];

                for(int j=result.size();j<keys.length;j++){
                    tmpKeys[keys.length-j-1]= keys[j];
                }

                result.addAll(getPop(i, pool, tmpKeys));
            }else{
                break;
            }
        }
    ...
    private List<Object> getPop(int i,VshardedJedisPool pool,String...keys){
        List<Object> result = new ArrayList<Object>();
        ...
        try{
            ...
            result.addAll(pipeline.syncAndReturnAlls(i));
